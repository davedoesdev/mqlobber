...
function MQlobberServer(fsq, stream, options)
{
    ...
    this._handler = function (data, info, cb)
    {
        ...
        if (!ths.emit('message', data, info, multiplex))
        {
            data.pipe(multiplex());
        }
    };
    ...
    this._handler.mqlobber_server = this;
...
MQlobberServer.prototype.subscribe = function (topic, cb)
{
    ...
    {
        this.fsq.subscribe(topic, this._handler, function (err)
        {
            if (!err)
            {
                ths._subs.add(topic);
...
MQlobberServer.prototype.unsubscribe = function (topic, cb)
{
    ...
    if (topic === undefined)
    {
        ...
        {
            ths.fsq.unsubscribe(t, ths._handler, function (err)
            {
                if (err)
                {
                    cb2(err);
    ...
    {
        this.fsq.unsubscribe(topic, this._handler, function (err)
        {
            if (!err)
            {
