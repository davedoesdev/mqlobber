...
function MQlobberServer(fsq, stream, options)
{
    ...
    this._handler = function (data, info, cb)
    {
        ...
        var duplex = mux.multiplex({ handshake_data: Buffer.concat(hdata) });
    ...
    mux.once('handshake', function (duplex, hdata, delay)
    {
        ...
        control.on('readable', function ()
        {
            ...
            if (data.length === 0)
            {
                return error.call(control, new Error('buffer too small'));
            }
        ...
        this.on('handshake', function (duplex, hdata, delay)
        {
            ...
            if (hdata.length < 1)
            {
                return warning.call(duplex, new Error('buffer too small'));
            }
            ...
            var topic = hdata.toString('utf8', pos),
                handshake = delay();
            ...
            if (!ths.emit('publish_requested', topic, duplex, options, published))
            {
                duplex.pipe(ths._fsq.publish(topic, options, published));
            }
        });
