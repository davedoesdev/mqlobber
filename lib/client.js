...
function MQlobberClient(stream, options)
{
    ...
    function end()
    {
        ths.emit('error', new Error('ended before handshaken'));
    }
...
MQlobberClient.prototype.subscribe = function (topic, handler, cb)
{
    ...
    if (this._done)
    {
        return done(new Error('finished'));
    }
...
MQlobberClient.prototype.unsubscribe = function (topic, handler, cb)
{
    ...
    if (this._done)
    {
        return done2(new Error('finished'));
    }
    ...
    duplex = this.mux.multiplex({ handshake_data: hdata });
...
MQlobberClient.prototype.publish = function (topic, options, cb)
{
    ...
    duplex = this.mux.multiplex({ handshake_data: Buffer.concat(hdata) });
